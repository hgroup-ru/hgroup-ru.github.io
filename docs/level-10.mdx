...
- First, see the section on the _[Sarcastic Discard](level-3.mdx#the-sarcastic-discard-sd)_.
...
- Oftentimes, doing a _Gentleman's Discard_ will delay things and cost the team _Tempo_. Thus, a _Gentleman's Discard_ that slows things down is only good if there is some other side benefit to offset the lost _Tempo_ (such as delaying the discard of a valuable card, for example).
...
- First, see the section on the _[Gentleman's Discard](#the-gentlemans-discard-gd)_.
...
- Note that it is **illegal** to perform a _Layered Baton Discard_; the card **must** be exactly on _Finesse Position_.
  ...
  - For level 23 players, a _Layered Baton Discard_ would instead be interpreted as a _[Blaze Discard](level-23.mdx#the-blaze-discard)_.
...
- Note that when giving a _Sarcastic Finesse_, the duplicated card in your own hand must be unknown. Otherwise, the clue receiver will simply think that the clued card is the next card in the suit.
...
- Sometimes, a player can accomplish the same outcome in two different ways:
  1. Giving a playable card a normal 1-for-1 _Play Clue_.
  1. Performing a 1-for-1 _Finesse_ that causes the same card to blind-play.
- In this situation, either line should work. But doing the line with the _Finesse_ is more complicated and carries more of a risk for the team, because _Finesses_ can be missed. Thus, it makes more sense to give the direct _Play Clue_ instead.
- This concept applies beyond _Finesses_; it should be applied to any complicated clue. We formalize this as the _Directness Principle_: if two lines would result in identical outcomes, always prefer the line with the least amount of complexity.
  - Specifically, "identical outcomes" means that in addition to the same cards being played, the superpositions on all clued cards would be the same.
