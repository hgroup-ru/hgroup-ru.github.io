...
- В «Пути обучения» пользовательские подписи `Level N` заменены на естественные русские формы `Уровень N` / «N-й уровень»; компактный селектор `L1–L25` намеренно не изменён.
